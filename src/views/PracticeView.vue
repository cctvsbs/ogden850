...
const selectedGender = ref('female'); const selectedAccent = ref('us'); const selectedRate = ref(1.0)
let isSpeaking = false; let lastSpokenText = ''; let speechTimeout = null
let resumeInterval = null  // 防暂停定时器
let activeUtterance = null // 防 GC
...
function selectBestVoice(gender, accent, voices) {
  ...
  const targetList = gender === 'female'
    ? (accent === 'uk' ? ['Microsoft Susan', 'Google UK English Female', 'Fiona', 'Moira', 'British Female'] : ['Microsoft Jenny', 'Google US English Female', 'Samantha', 'Microsoft Zira', 'Karen'])
    : (accent === 'uk' ? ['Microsoft George', 'Google UK English Male', 'Oliver', 'British Male', 'Daniel'] : ['Microsoft Guy', 'Google US English Male', 'Tom', 'Microsoft David', 'Alex'])
...
function speak() {
  ...
  activeUtterance.onstart = () => {
    // 每 500ms 捅一下 resume，防止 Edge 神经语音自动暂停
  ...
  activeUtterance.onend = () => {
    isSpeaking = false; activeUtterance = null; clearAllTimers()
  }
  ...
  speechTimeout = setTimeout(() => {
    if (isSpeaking) {
      isSpeaking = false; clearAllTimers()
      window.speechSynthesis.cancel()
    }
  }, 5000)
...
function generateOptions(correctWord) { const others = store.words.filter(w => w.id !== correctWord.id); const shuffled = [...others].sort(() => Math.random() - 0.5); const distractors = shuffled.slice(0, 3).map(w => w.chinese); return [{ text: correctWord.chinese, correct: true }, ...distractors.map(t => ({ text: t, correct: false }))].sort(() => Math.random() - 0.5) }
function initChoiceMode() { const pool = store.dueWords.length > 0 ? [...store.dueWords] : [...store.unmasteredWords]; const shuffled = pool.sort(() => Math.random() - 0.5); choiceQueue.value = shuffled.slice(0, roundSize); choiceIndex.value = 0; choiceOptions.value = generateOptions(choiceQueue.value[0]); highlightedOption.value = null; confirmedOption.value = null; isAnswered.value = false; choiceStats.value = { correct: 0, wrong: 0 } }
...
// ===== 拼写 =====
const spellingQueue = ref([]); const spellingIndex = ref(0); const userInput = ref(''); const isSpellingSubmitted = ref(false); const isSpellingCorrect = ref(false); const showHint = ref(false)
const hidePhonetic = ref(true)
const spellingStats = ref({ correct: 0, wrong: 0, hints: 0 }); const inputRef = ref(null); const spellingRoundSize = 10
const spellingHintLetters = ref([])

function initSpellingMode() { const pool = store.dueWords.length > 0 ? [...store.dueWords] : [...store.unmasteredWords]; const shuffled = pool.sort(() => Math.random() - 0.5); spellingQueue.value = shuffled.slice(0, spellingRoundSize); spellingIndex.value = 0; userInput.value = ''; isSpellingSubmitted.value = false; isSpellingCorrect.value = false; showHint.value = false; spellingStats.value = { correct: 0, wrong: 0, hints: 0 }; spellingHintLetters.value = []; hidePhonetic.value = true; nextTick(() => inputRef.value?.focus()) }
...
const letterChecks = computed(() => { if (!currentSpelling.value) return []; const target = currentSpelling.value.word; const input = userInput.value; return target.split('').map((char, i) => ({ char: target[i], inputChar: input[i] || '', status: i >= input.length ? 'pending' : (input[i].toLowerCase() === char.toLowerCase() ? 'correct' : 'wrong'), hinted: spellingHintLetters.value.includes(i) })) })
...
function submitSpelling() { if (isSpellingSubmitted.value || !currentSpelling.value) return; const correct = userInput.value.trim().toLowerCase() === currentSpelling.value.word.toLowerCase(); isSpellingSubmitted.value = true; isSpellingCorrect.value = correct; if (correct) { spellingStats.value.correct++; store.updateWordProgress(currentSpelling.value.id, { stage: (currentSpelling.value.stage || 0) + 1, srs: { ...currentSpelling.value.srs, nextReview: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString() } }); mistakesService.removeMistakeByWordId(currentSpelling.value.id) } else { spellingStats.value.wrong++; mistakesService.addMistake(currentSpelling.value.id, 'spelling', userInput.value.trim(), currentSpelling.value.word, currentSpelling.value.word, currentSpelling.value.chinese) }; playChoiceSound(correct); saveAllProgress(); setTimeout(() => { if (spellingIndex.value + 1 >= spellingQueue.value.length) { spellingIndex.value++; saveAllProgress(); setTimeout(() => playFinishSound(), 100) } else { spellingIndex.value++; userInput.value = ''; isSpellingSubmitted.value = false; isSpellingCorrect.value = false; showHint.value = false; spellingHintLetters.value = []; hidePhonetic.value = true; nextTick(() => inputRef.value?.focus()) } }, 1200) }
...
// ===== 表达 =====
const expressionTasks = ref([]); const expressionIndex = ref(0); const expressionInput = ref(''); const expressionSubmitted = ref(false); const expressionFeedback = ref(''); const expressionStats = ref({ completed: 0, total: 0 }); const expressionInputRef = ref(null); const expressionRoundSize = 5
const expressionHintWords = ref([]); const expressionHintWord = ref('')
...
function parseAnswerToWords(answer) { const rawWords = answer.split(/\s+/).filter(Boolean); return rawWords.map(w => { const letters = w.split(''); return { text: w, letters } }) }
const expressionLetterSlots = computed(() => {
  if (!currentExpression.value) return { words: [], currentSlot: null }
  const words = parseAnswerToWords(currentExpression.value.answer); const input = expressionInput.value; let inputPos = 0; let currentSlot = null
  const result = words.map((word, wi) => {
    const slots = word.letters.map((char, si) => {
      const isLetter = /[a-zA-Z]/.test(char)
      if (!isLetter) return { char, inputChar: char, status: 'auto', position: si, globalPos: inputPos }
      const inputChar = input[inputPos] || ''
      const status = inputPos >= input.length ? 'pending' : inputChar.toLowerCase() === char.toLowerCase() ? 'correct' : 'wrong'
      const slot = { char, inputChar: inputChar || '', status, position: si, globalPos: inputPos }
      if (inputPos === expressionInput.value.replace(/[^a-zA-Z]/g, '').length && status === 'pending' && !currentSlot) currentSlot = { wordIndex: wi, letterIndex: si }
      inputPos++; return slot
    }); return { text: word.text, slots }
  }); return { words: result, currentSlot }
})
function isCurrentSlot(wi, si) { const cs = expressionLetterSlots.value.currentSlot; return cs && cs.wordIndex === wi && cs.letterIndex === si }

function initExpressionMode() { const shuffled = [...expressionBank].sort(() => Math.random() - 0.5); expressionTasks.value = shuffled.slice(0, expressionRoundSize); expressionIndex.value = 0; expressionInput.value = ''; expressionSubmitted.value = false; expressionFeedback.value = ''; expressionStats.value = { completed: 0, total: expressionRoundSize }; expressionHintWords.value = []; expressionHintWord.value = ''; nextTick(() => expressionInputRef.value?.focus()) }
function showExpressionHint() { if (expressionSubmitted.value || !currentExpression.value) return; const words = parseAnswerToWords(currentExpression.value.answer); const hintedSet = new Set(expressionHintWords.value); const unhintedIndices = words.map((_, i) => i).filter(i => !hintedSet.has(i)); if (unhintedIndices.length === 0) return; const randomIdx = unhintedIndices[Math.floor(Math.random() * unhintedIndices.length)]; expressionHintWords.value = [...expressionHintWords.value, randomIdx]; expressionHintWord.value = words[randomIdx].text }
function clearExpressionHints() { expressionHintWords.value = []; expressionHintWord.value = '' }
function submitExpression() { if (expressionSubmitted.value || !currentExpression.value) return; const answer = expressionInput.value.trim(); if (!answer) return; expressionSubmitted.value = true; const task = currentExpression.value; const keywordsFound = task.keywords.filter(kw => answer.toLowerCase().includes(kw.toLowerCase())); const score = keywordsFound.length / task.keywords.length; let feedback = ''; if (score >= 0.8) { feedback = `🌟 很好！\n参考表达：${task.answer}`; playChoiceSound(true) } else if (score >= 0.5) { feedback = `👍 不错！使用了 ${keywordsFound.length}/${task.keywords.length} 个核心词。\n参考表达：${task.answer}`; playChoiceSound(true) } else { feedback = `💡 试试这样表达：${task.answer}`; playChoiceSound(false) }; expressionFeedback.value = feedback; expressionStats.value.completed++; saveAllProgress(); setTimeout(() => { if (expressionIndex.value + 1 >= expressionTasks.value.length) { expressionIndex.value++; saveAllProgress(); setTimeout(() => playFinishSound(), 100) } else { expressionIndex.value++; expressionInput.value = ''; expressionSubmitted.value = false; expressionFeedback.value = ''; expressionHintWords.value = []; expressionHintWord.value = ''; nextTick(() => expressionInputRef.value?.focus()) } }, 2500) }
...
function handleKeydown(e) {
  if ((e.key === 's' || e.key === 'S') && document.activeElement?.tagName !== 'INPUT' && document.activeElement?.tagName !== 'TEXTAREA') { e.preventDefault(); speak(); return }
  if (mode.value === 'expression') { if (expressionFinished.value) { if (e.key === 'r' || e.key === 'R') restartExpression(); return }; if (!expressionSubmitted.value && e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); submitExpression() }; return }
  if (mode.value === 'spelling') { if (spellingFinished.value) { if (e.key === 'r' || e.key === 'R') restartSpelling(); return }; if (!isSpellingSubmitted.value) { if (e.key === 'Enter') { e.preventDefault(); submitSpelling() }; if (e.key === 'Tab') { e.preventDefault(); showSpellingHint() } }; return }
  if (mode.value === 'flashcard') { if (flashcardFinished.value) { if (e.key === 'r' || e.key === 'R') restartFlashcard(); return }; if (e.key === ' ' || e.code === 'Space') { e.preventDefault(); toggleFlip(); return }; if (e.key === 'ArrowLeft') { e.preventDefault(); prevCard(); return }; if (e.key === 'ArrowRight') { e.preventDefault(); nextCard(); return }; if (isFlipped.value) { if (e.key === '1') { e.preventDefault(); applyGrade('again'); return }; if (e.key === '2') { e.preventDefault(); applyGrade('hard'); return }; if (e.key === '3') { e.preventDefault(); applyGrade('good'); return }; if (e.key === '4') { e.preventDefault(); applyGrade('easy'); return } }; return }
...
onMounted(async () => {
  await store.loadProgress(); loadAllProgress(); loadOfflineMode(); initVoices()
  window.addEventListener('keydown', handleKeydown)
  if (queue.value.length > 0 && !isOffline.value) loadImageForCurrent()
  if (queryMode) { if (queryMode === 'choice' && choiceQueue.value.length === 0) initChoiceMode(); if (queryMode === 'spelling' && spellingQueue.value.length === 0) initSpellingMode(); if (queryMode === 'expression' && expressionTasks.value.length === 0) initExpressionMode() }
})
onUnmounted(() => {
  ...
  <div class="w-full max-w-2xl mx-auto px-3 sm:px-4 py-4 sm:py-8">
    ...
    <template v-if="mode === 'flashcard'">
      ...
      <template v-if="!flashcardFinished && currentCard">
        <div class="mb-4 sm:mb-6 cursor-pointer" style="perspective: 1400px;" @click="toggleFlip"><div class="relative w-full transition-transform duration-500" style="transform-style: preserve-3d; min-height: 560px;" :style="{ transform: isFlipped ? 'rotateY(180deg)' : '' }">
        ...
        <div class="flex justify-center items-center gap-3 mb-3 sm:mb-4 flex-wrap">
          ...
          <select v-model="selectedAccent" @change="updateVoice" class="px-2 py-1.5 rounded-full text-xs cursor-pointer border-none" :style="{ backgroundColor: 'var(--bg-secondary)', color: 'var(--text)' }"><option value="us">🇺🇸</option><option value="uk">🇬🇧</option></select>
          <select v-model="selectedRate" class="px-2 py-1.5 rounded-full text-xs cursor-pointer border-none" :style="{ backgroundColor: 'var(--bg-secondary)', color: 'var(--text)' }"><option :value="0.5">0.5x</option><option :value="0.75">0.75x</option><option :value="1.0">1x</option><option :value="1.25">1.25x</option><option :value="1.5">1.5x</option></select>
          <select v-model="selectedGender" @change="updateVoice" class="px-2 py-1.5 rounded-full text-xs cursor-pointer border-none" :style="{ backgroundColor: 'var(--bg-secondary)', color: 'var(--text)' }"><option value="female">👩</option><option value="male">👨</option></select>
        ...
        <p class="text-center text-xs opacity-60 mb-2" :style="{ color: 'var(--text)' }">← <kbd>A</kbd> 上一张 · 下一张 <kbd>D</kbd> → · 🔊 <kbd>S</kbd> 发音 · ⭐ <kbd>F</kbd> 收藏</p>
        ...
        <p class="text-center text-[10px] sm:text-xs mt-3 sm:mt-4 opacity-60" :style="{ color: 'var(--text-muted)' }">🎯 SM-2 · <kbd>1</kbd> Again · <kbd>2</kbd> Hard · <kbd>3</kbd> Good · <kbd>4</kbd> Easy</p>
      </template>
    ...
    <template v-if="mode === 'choice'">
      ...
      <template v-if="!choiceFinished && currentChoice"><div class="rounded-2xl sm:rounded-3xl p-6 sm:p-10 mb-4 sm:mb-6 text-center" :style="{ backgroundColor: 'var(--bg)', boxShadow: '0 10px 25px -10px rgba(0,0,0,0.08)' }"><p class="text-xs sm:text-sm mb-2" :style="{ color: 'var(--text-muted)' }">以下单词的中文意思是？</p><p class="text-4xl sm:text-5xl font-bold word-display" :style="{ color: 'var(--text)' }">{{ currentChoice.word }}</p><p class="text-base sm:text-lg mt-2" :style="{ color: 'var(--text-muted)' }">{{ currentChoice.phonetic }}</p></div>
        ...
        <div class="grid grid-cols-1 gap-2 sm:gap-3"><button v-for="(option, i) in choiceOptions" :key="i" @click="selectChoice(i)" class="w-full text-left px-4 sm:px-6 py-3 sm:py-4 rounded-xl sm:rounded-2xl text-base sm:text-lg font-medium transition-all cursor-pointer" :class="{ 'cursor-default': isAnswered }" :style="isAnswered ? (option.correct ? { backgroundColor: '#e3f3e0', color: '#2b7551', border: '2px solid #5fad56' } : (confirmedOption === i ? { backgroundColor: '#fdeaec', color: '#b13e3e', border: '2px solid #dc6b4a' } : { backgroundColor: 'var(--bg-secondary)', color: 'var(--text-muted)', opacity: '0.5' })) : (highlightedOption === i ? { backgroundColor: 'var(--bg-secondary)', color: 'var(--text)', border: '2px solid var(--accent)', boxShadow: '0 0 0 2px var(--accent)' } : { backgroundColor: 'var(--bg)', color: 'var(--text)', border: '2px solid var(--border)' })"><span class="inline-block w-6 h-6 sm:w-7 sm:h-7 rounded-full text-xs sm:text-sm text-center leading-6 sm:leading-7 mr-2 sm:mr-3 font-bold" :style="isAnswered && option.correct ? { backgroundColor: '#5fad56', color: '#fff' } : (isAnswered && confirmedOption === i ? { backgroundColor: '#dc6b4a', color: '#fff' } : (highlightedOption === i ? { backgroundColor: 'var(--accent)', color: '#1A1A2E' } : { backgroundColor: 'var(--bg-secondary)', color: 'var(--text-muted)' }))">{{ i + 1 }}</span>{{ option.text }}</button></div>
        <div class="flex justify-center mt-4 sm:mt-6 gap-2 sm:gap-3 items-center"><select v-model="selectedRate" class="px-2 py-1.5 rounded-full text-xs cursor-pointer border-none" :style="{ backgroundColor: 'var(--bg-secondary)', color: 'var(--text)' }"><option :value="0.5">0.5x</option><option :value="0.75">0.75x</option><option :value="1.0">1x</option><option :value="1.25">1.25x</option><option :value="1.5">1.5x</option></select><button @click="speak" class="px-3 py-1.5 rounded-full text-xs cursor-pointer" :style="{ backgroundColor: 'var(--bg-secondary)', color: 'var(--text)' }" title="朗读 (S)">🔊</button><button @click.stop="toggleFavChoice" class="px-3 py-1.5 rounded-full text-xs cursor-pointer" :style="{ backgroundColor: 'var(--bg-secondary)', color: isFavChoice ? '#f59e0b' : 'var(--text)' }" :title="isFavChoice ? '取消收藏' : '添加收藏'">{{ isFavChoice ? '⭐' : '☆' }}</button></div>
      </template>
    ...
    <template v-if="mode === 'spelling'">
      ...
      <template v-if="!spellingFinished && currentSpelling">
        <div class="rounded-2xl sm:rounded-3xl p-6 sm:p-10 mb-4 sm:mb-6 text-center" :style="{ backgroundColor: 'var(--bg)', boxShadow: '0 10px 25px -10px rgba(0,0,0,0.08)' }"><p class="text-xs sm:text-sm mb-2" :style="{ color: 'var(--text-muted)' }">请拼写以下中文对应的英文单词</p><p class="text-3xl sm:text-4xl font-bold mb-2 word-display" :style="{ color: 'var(--text)' }">{{ currentSpelling.chinese }}</p><p v-if="!hidePhonetic" class="text-base sm:text-lg" :style="{ color: 'var(--text-muted)' }">{{ currentSpelling.phonetic }}</p><button v-if="hidePhonetic && !isSpellingSubmitted" @click="hidePhonetic = false" class="text-xs mt-1 underline cursor-pointer" :style="{ color: 'var(--text-muted)' }">👁️ 显示音标</button></div>
        <div v-if="!isSpellingSubmitted" class="flex justify-center gap-1 sm:gap-1.5 mb-3 sm:mb-4 flex-wrap"><span v-for="(check, i) in letterChecks" :key="i" class="inline-flex items-center justify-center w-7 h-9 sm:w-8 sm:h-10 rounded-lg text-base sm:text-lg font-bold font-mono transition-colors" :style="check.status === 'correct' ? { backgroundColor: '#e3f3e0', color: '#2b7551' } : check.status === 'wrong' ? { backgroundColor: '#fdeaec', color: '#b13e3e' } : check.hinted ? { backgroundColor: '#fff0e0', color: '#c96f0e', border: '2px dashed #c96f0e' } : { backgroundColor: 'var(--bg-secondary)', color: 'var(--text-muted)' }">{{ check.inputChar || (check.hinted ? check.char : '_') }}</span></div>
        <div v-if="!isSpellingSubmitted" class="relative"><input ref="inputRef" v-model="userInput" @keydown.enter.prevent="submitSpelling" placeholder="输入英文单词..." class="w-full px-4 sm:px-6 py-4 sm:py-5 text-xl sm:text-2xl text-center rounded-xl sm:rounded-2xl border-2 outline-none transition-all font-mono tracking-wider" :style="{ backgroundColor: 'var(--bg)', borderColor: 'var(--border)', color: 'var(--text)' }" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" /></div>
        <div v-if="isSpellingSubmitted" class="text-center py-4"><p v-if="isSpellingCorrect" class="text-xl sm:text-2xl font-bold" :style="{ color: 'var(--success)' }">✅ 正确！</p><div v-else><p class="text-xl sm:text-2xl font-bold mb-2" :style="{ color: 'var(--danger)' }">❌ 错误</p><p class="text-base sm:text-lg" :style="{ color: 'var(--text-muted)' }">你的输入：<span :style="{ color: 'var(--danger)' }">{{ userInput }}</span></p><p class="text-base sm:text-lg">正确答案：<span :style="{ color: 'var(--success)' }">{{ currentSpelling.word }}</span></p></div></div>
        <p class="text-center text-xs mt-2 mb-2 opacity-60" :style="{ color: 'var(--text)' }"><kbd>Enter</kbd> 确认 · <kbd>Tab</kbd> 提示 · <kbd>S</kbd> 发音</p>
        <div class="flex justify-center mt-2 sm:mt-3 gap-2 sm:gap-3 items-center flex-wrap"><button v-if="!isSpellingSubmitted" @click="hidePhonetic = !hidePhonetic" class="px-3 py-1.5 rounded-full text-xs font-medium cursor-pointer transition-all hover:opacity-80" :style="{ backgroundColor: hidePhonetic ? 'var(--accent)' : 'var(--bg-secondary)', color: hidePhonetic ? '#1A1A2E' : 'var(--text)' }">{{ hidePhonetic ? '👁️ 音标' : '🙈 音标' }}</button><button v-if="!isSpellingSubmitted" @click="showSpellingHint" class="px-3 py-1.5 rounded-full text-xs font-medium cursor-pointer transition-all hover:opacity-80" :style="{ backgroundColor: '#fff0e0', color: '#c96f0e' }" title="提示随机字母 (Tab)">💡 提示</button><button v-if="!isSpellingSubmitted && spellingHintLetters.length > 0" @click="clearSpellingHints" class="px-3 py-1.5 rounded-full text-xs font-medium cursor-pointer transition-all hover:opacity-80" :style="{ backgroundColor: 'var(--bg-secondary)', color: 'var(--text-secondary)' }" title="清除所有提示">🗑️ 清空</button><button v-if="!isSpellingSubmitted" @click="submitSpelling" class="px-4 py-1.5 rounded-full text-xs font-medium cursor-pointer transition-all hover:opacity-80" :style="{ backgroundColor: 'var(--accent)', color: '#1A1A2E' }">↵ 确认</button><button @click="speak" class="px-3 py-1.5 rounded-full text-xs cursor-pointer" :style="{ backgroundColor: 'var(--bg-secondary)', color: 'var(--text)' }" title="朗读 (S)">🔊</button><button @click.stop="toggleFavSpelling" class="px-3 py-1.5 rounded-full text-xs cursor-pointer" :style="{ backgroundColor: 'var(--bg-secondary)', color: isFavSpelling ? '#f59e0b' : 'var(--text)' }" :title="isFavSpelling ? '取消收藏' : '添加收藏'">{{ isFavSpelling ? '⭐' : '☆' }}</button></div>
      </template>
    ...
    <template v-if="mode === 'expression'">
      ...
      <template v-if="!expressionFinished && currentExpression">
        <div class="rounded-2xl sm:rounded-3xl p-6 sm:p-10 mb-4 sm:mb-6 text-center" :style="{ backgroundColor: 'var(--bg)', boxShadow: '0 10px 25px -10px rgba(0,0,0,0.08)' }"><p class="text-xs sm:text-sm mb-2 sm:mb-3" :style="{ color: 'var(--text-muted)' }">用 Ogden 850 基础词汇表达以下意思：</p><p class="text-2xl sm:text-3xl font-bold mb-2" :style="{ color: 'var(--text)' }">{{ currentExpression.zh }}</p></div>
        <div v-if="!expressionSubmitted" class="mb-4">
          <div v-if="expressionHintWords.length > 0" class="flex flex-wrap justify-center gap-2 mb-3"><span v-for="(wordIdx, i) in expressionHintWords" :key="i" class="px-2.5 py-1 rounded-lg text-sm font-medium" :style="{ backgroundColor: '#fff0e0', color: '#c96f0e' }">{{ expressionLetterSlots.words[wordIdx]?.text || '' }}</span></div>
          <div class="flex flex-wrap justify-center gap-x-4 gap-y-2"><div v-for="(word, wi) in expressionLetterSlots.words" :key="wi" class="flex gap-0.5"><span v-for="(slot, si) in word.slots" :key="si" class="inline-flex items-center justify-center w-7 h-9 sm:w-8 sm:h-10 rounded-md text-base sm:text-lg font-bold font-mono transition-all" :class="{ 'ring-2 ring-[var(--accent)]': !expressionSubmitted && isCurrentSlot(wi, si) }" :style="slot.status === 'auto' ? { backgroundColor: 'transparent', color: 'var(--text-muted)', border: '1px dashed transparent', fontWeight: '400' } : slot.status === 'correct' ? { backgroundColor: '#e3f3e0', color: '#2b7551', border: '1px solid #5fad56' } : slot.status === 'wrong' ? { backgroundColor: '#fdeaec', color: '#b13e3e', border: '1px solid #dc6b4a' } : { backgroundColor: 'var(--bg-secondary)', color: 'var(--text-muted)', border: '1px dashed var(--border)' }">{{ slot.inputChar || (slot.status === 'auto' ? slot.char : '_') }}</span></div></div>
        </div>
        <p v-if="!expressionSubmitted" class="text-center text-xs mb-2 opacity-60" :style="{ color: 'var(--text)' }"><kbd>Enter</kbd> 提交 · <kbd>S</kbd> 发音</p>
        <div v-if="!expressionSubmitted"><textarea ref="expressionInputRef" v-model="expressionInput" @keydown.enter.exact.prevent="submitExpression" placeholder="输入英文翻译..." rows="3" class="w-full px-4 sm:px-6 py-4 sm:py-5 text-lg sm:text-xl rounded-xl sm:rounded-2xl border-2 outline-none transition-all resize-none" :style="{ backgroundColor: 'var(--bg)', borderColor: 'var(--border)', color: 'var(--text)' }"></textarea>
          <div class="flex justify-center mt-3 sm:mt-4 gap-2 sm:gap-3 items-center flex-wrap"><select v-model="selectedRate" class="px-2 py-1.5 rounded-full text-xs cursor-pointer border-none" :style="{ backgroundColor: 'var(--bg-secondary)', color: 'var(--text)' }"><option :value="0.5">0.5x</option><option :value="0.75">0.75x</option><option :value="1.0">1x</option><option :value="1.25">1.25x</option><option :value="1.5">1.5x</option></select><button @click="speak" class="px-3 py-1.5 rounded-full text-xs font-medium cursor-pointer transition-all hover:opacity-80" :style="{ backgroundColor: 'var(--bg-secondary)', color: 'var(--text)' }" title="听标准表达 (S)">🔊</button><button @click="showExpressionHint" class="px-3 py-1.5 rounded-full text-xs font-medium cursor-pointer transition-all hover:opacity-80" :style="{ backgroundColor: '#fff0e0', color: '#c96f0e' }" title="提示随机单词">💡 提示</button><button v-if="expressionHintWords.length > 0" @click="clearExpressionHints" class="px-3 py-1.5 rounded-full text-xs font-medium cursor-pointer transition-all hover:opacity-80" :style="{ backgroundColor: 'var(--bg-secondary)', color: 'var(--text-secondary)' }" title="清除所有提示">🗑️ 清空</button><button @click="submitExpression" class="px-4 py-2 rounded-full text-xs font-semibold cursor-pointer transition-all hover:scale-105" :style="{ backgroundColor: 'var(--accent)', color: '#1A1A2E' }">↵ 提交</button></div></div>
        <div v-if="expressionSubmitted" class="mt-3 sm:mt-4 p-4 sm:p-6 rounded-2xl" :style="{ backgroundColor: 'var(--bg-secondary)' }"><p class="text-xs sm:text-sm mb-2" :style="{ color: 'var(--text-muted)' }">你的回答：</p><p class="text-base sm:text-lg mb-3 sm:mb-4" :style="{ color: 'var(--text)' }">{{ expressionInput }}</p><div class="border-t pt-3 sm:pt-4" :style="{ borderColor: 'var(--border)' }"><p class="text-sm sm:text-base font-medium mb-1" :style="{ color: 'var(--accent)' }">📝 反馈：</p><p class="text-sm sm:text-base whitespace-pre-line" :style="{ color: 'var(--text-secondary)' }">{{ expressionFeedback }}</p></div></div>
      </template>
    ...
    <!-- 快捷键浮层 -->
    <div class="fixed bottom-4 right-4 px-4 py-2 rounded-2xl text-xs z-50 pointer-events-none hidden sm:block" :style="{ backgroundColor: 'rgba(30,30,30,0.85)', color: '#e3dccc', backdropFilter: 'blur(8px)' }">
      <p class="font-bold mb-1">⌨️ 快捷键</p>
      <p><kbd>S</kbd> 发音 <kbd>R</kbd> 重新开始</p>
      <template v-if="mode === 'flashcard'"><p><kbd>空格</kbd> 翻面 <kbd>←→</kbd> 切换</p><p><kbd>1</kbd> Again <kbd>2</kbd> Hard <kbd>3</kbd> Good <kbd>4</kbd> Easy</p></template>
      <template v-if="mode === 'choice'"><p><kbd>1-4</kbd> 选择 <kbd>Enter</kbd> 确认 <kbd>Esc</kbd> 取消</p></template>
      <template v-if="mode === 'spelling'"><p><kbd>Enter</kbd> 确认 <kbd>Tab</kbd> 提示 <kbd>S</kbd> 发音</p></template>
      <template v-if="mode === 'expression'"><p><kbd>Enter</kbd> 提交 <kbd>S</kbd> 发音</p></template>
    </div>
  </div>
</template>
